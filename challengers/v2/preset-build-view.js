...
`;
  ...
  function isExcludedBoss(boss) {
    return boss?.series === 'verus-hilla';
  }
  ...
  function decorateKeyBossChips(card) {
    card.querySelectorAll('.preset-boss-chip:not(.more)').forEach((chip) => {
      ...
      if (isExcludedBoss(boss)) chip.classList.add('is-excluded');
      chip.innerHTML = `${bossIcon(boss, 28)}<span>${escapeHtml(label)}</span>${isExcludedBoss(boss) ? exclusionBadgeHtml() : ''}`;
    });
  }
    ...
    return `
      <div class="preset-flex-note" data-preset-flex-note>
        ...
        <div class="preset-flex-list">
          ...
          ${excludedBosses.map((boss) => `<span class="has-boss-icon excluded">${bossIcon(boss, 24)}<span>${escapeHtml(bossLabel(boss))}</span>${exclusionBadgeHtml()}</span>`).join('')}
        </div>
      </div>`;
  }
          ...
          ${groups.map(([key, bosses]) => `
            <div class="preset-build-group">
              ...
              <div class="preset-build-boss-list">
                ${bosses.map((boss) => `
                  <span class="preset-build-boss${boss.id === 'dusk-chaos' ? ' preferred' : ''}${equalDifficultySet.has(boss.id) ? ' equal' : ''}${isExcludedBoss(boss) ? ' excluded' : ''}">
                    ${bossIcon(boss, 34)}
                    <span class="preset-build-boss-label">${escapeHtml(bossLabel(boss))}${isExcludedBoss(boss) ? exclusionBadgeHtml() : ''}</span>
                    <small>${number.format(boss.points)}점${boss.id === 'dusk-chaos' ? ' · 우선 추천' : ''}${isExcludedBoss(boss) ? ' · 제외군' : ''}</small>
                  </span>`).join('')}
              </div>
            </div>`).join('')}
        </div>
